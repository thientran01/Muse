...
| `ring-accent/25` (`TokenList.tsx:39`) | → focus token. The alpha is not the defect. **Correction (found in review):** this is *not* the only control on `focus:` — `ColorPicker.tsx:377` and `PropertiesPanel.tsx:1039` were too. The rule that actually holds: **ring**-outlined controls use `focus-visible:` (all 21 of them, incl. `:1039`, whose resting state is `ring-1`); the four **border**-outlined text inputs keep `focus:`/`focus-within:`, since a clicked text field is expected to highlight. |
